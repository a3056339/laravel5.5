<?php
namespace App\Libraries\Alipaypay\aop;
use Exception;
...
/** 
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function checkSignAndali_decrypt($params, $rsaPublicKeyPem, $rsaPrivateKeyPem, $isCheckSign, $isDecrypt, $signType='RSA') {
		...
		if ($isDecrypt) {
			return $this->rsaali_decrypt($bizContent, $rsaPrivateKeyPem, $charset);
		}
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function rsaali_decrypt($data, $rsaPrivateKeyPem, $charset) {
		...
		foreach ($decodes as $n => $decode) {
			if (!openssl_private_ali_decrypt($decode, $dcyCont, $res)) {
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$strnull .= $dcyCont;
		}
